...
import {
  Initialized,
  LeverageOrderCancelled,
  LeverageOrderExecuted,
  LeverageOrderExpired,
  LeverageOrderPlaced,
  OracleChanged,
  Swap,
  SwapOrderCancelled,
...
import { ValueDecimals, _getPrice, loadOrCreateProtocol, loadOrCreateUser } from "./helper";
import { Oracle as OracleTemplate } from "../generated/templates";

export function handleOrderPlaced(ev: LeverageOrderPlaced): void {
...
export function handleOrderCancelled(ev: LeverageOrderCancelled): void {
...
export function handleOrderExpired(ev: LeverageOrderExpired): void {
  let order = Order.load(ev.params.key.toHex());
  if (!order) {
    return;
  }
...
export function handleOrderExecuted(ev: LeverageOrderExecuted): void {
  let order = Order.load(ev.params.key.toHex());
  if (!order) {
    return;
  }
...
export function handleSwapOrderPlaced(ev: SwapOrderPlaced): void {
  ...
  order.owner = ev.params.order.owner;
  order.tokenIn = ev.params.order.tokenIn;
  order.tokenOut = ev.params.order.tokenOut;
  order.amountIn = ev.params.order.amountIn;
  order.minAmountOut = ev.params.order.minAmountOut;
  order.price = ev.params.order.price;
  order.executionPrice = integer.ZERO;
  order.executionFee = ev.params.order.executionFee;
  ...
  if (receipt) {
    ...
    if (filterSwapLog) {
      ...
      params = ethereum
        ...
        .toTuple();
      let swapFee = params[4].toBigInt();
      let tokenOut = params[1].toAddress();
