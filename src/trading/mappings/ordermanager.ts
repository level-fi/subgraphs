...
import { Oracle as OracleContract } from "../generated/Pool/Oracle";
import { PriceFeed as PriceFeedContract } from "../generated/Pool/PriceFeed";

export function handleOrderPlaced(ev: OrderPlaced): void {
  let orderContract = OrderManager.bind(ev.address);
  let oracleAddress = getOrNull<Address>(orderContract.try_oracle());
  let oracleContract = OracleContract.bind(
    oracleAddress ? oracleAddress : Address.fromString(ZERO_ADDRESS)
  );
  let priceFeed = PriceFeedContract.bind(
    oracleAddress ? oracleAddress : Address.fromString(ZERO_ADDRESS)
  );
  let collateralPrice = isNewOracleStart(ev)
    ? getOrNull<BigInt>(
        oracleContract.try_getPrice(ev.params.order.collateralToken, false)
      )
    : getOrNull<BigInt>(
        priceFeed.try_getLastPrice(ev.params.order.collateralToken)
      );
