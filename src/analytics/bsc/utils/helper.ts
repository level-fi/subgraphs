...
export const isTestnet = dataSource.network() == "chapel";
...
export function getDayId(timestamp: BigInt): BigInt {
  const dayTimestamp = ((timestamp.toI32() + 10 * 3600) / 86400) * 86400;
...
export function getOneWeekId(timestamp: BigInt): BigInt {
  const weekTimestamp =
    timestamp.toI32() +
    10 * 3600 -
    ((timestamp.toI32() + 10 * 3600 - 86400 * 4) % (86400 * 7));
...
export function loadOrCreateTranche(tranche: Address): Tranche {
  ...
  if (entity === null) {
    ...
    entity.tradingFeeValue = ZERO;
    entity.swapFeeValue = ZERO;
    entity.mintFeeValue = ZERO;
    entity.burnFeeValue = ZERO;
    entity.liquidateFeeValue = ZERO;
    entity.traderPnl = ZERO;
    entity.returnFee = ZERO;
    entity.traderFee = ZERO;
    entity.tranche = tranche;
  }
  return entity;
}

export function loadOrCreateTrancheFee(
  tranche: Address,
  token: Address
): TrancheFee {
  let entity = TrancheFee.load(`${tranche.toHex()}-${token.toHex()}`);
  if (entity === null) {
    entity = new TrancheFee(`${tranche.toHex()}-${token.toHex()}`);
    entity.tranche = tranche;
    entity.token = token;
    entity.trading = ZERO;
    entity.swap = ZERO;
...
export function loadOrCreateTrancheStat(
  ...
  if (entity === null) {
    ...
    entity.tradingFeeValue = DECIMAL_ZERO;
    entity.swapFeeValue = DECIMAL_ZERO;
    entity.mintFeeValue = DECIMAL_ZERO;
    entity.burnFeeValue = DECIMAL_ZERO;
    entity.liquidateFeeValue = DECIMAL_ZERO;
    entity.traderPnl = DECIMAL_ZERO;
    entity.cumulativeTraderPnl = DECIMAL_ZERO;
    entity.returnFee = DECIMAL_ZERO;
    entity.cumulativeReturnFee = DECIMAL_ZERO;
    entity.traderFee = DECIMAL_ZERO;
    entity.cumulativeTraderFee = DECIMAL_ZERO;
    entity.tranche = tranche;
    entity.period = period;
...
export function loadOrCreateDaoStaking(
  id: string,
  period: string,
  timestamp: BigInt
): DaoStaking {
  let entity = DaoStaking.load(id);
  if (entity === null) {
    entity = new DaoStaking(id);
    entity.stake = DECIMAL_ZERO;
    entity.unStake = DECIMAL_ZERO;
    entity.cumulative = DECIMAL_ZERO;
    entity.period = period;
    entity.timestamp = timestamp.toI32();
  }
...
export function loadOrCreateLvlStaking(
  id: string,
  period: string,
  timestamp: BigInt
): LvlStaking {
  let entity = LvlStaking.load(id);
  if (entity === null) {
    entity = new LvlStaking(id);
    entity.stake = DECIMAL_ZERO;
    entity.unStake = DECIMAL_ZERO;
    entity.cumulative = DECIMAL_ZERO;
    entity.period = period;
...
export function loadOrCreateLgoStaking(
  id: string,
  period: string,
  timestamp: BigInt
): LgoStaking {
  let entity = LgoStaking.load(id);
  if (entity === null) {
    entity = new LgoStaking(id);
    entity.stake = DECIMAL_ZERO;
    entity.unStake = DECIMAL_ZERO;
    entity.cumulative = DECIMAL_ZERO;
    entity.period = period;
...
export function _needUpdate(block: BigInt): boolean {
  ...
  return block.minus(protocol.lastUpdatedBlock).ge(BigInt.fromI32(50));
}
